...
xls = common.get_xls("pc.xlsx", "判断是否抢单")  # excel取值
...
class RecieveOrNot(unittest.TestCase):
    def setParameters(self, case_name, url, method, consultationId,consultationStateList,customerId,msg):
        self.case_name = str(case_name)
        self.method = str(method)
        self.consultationStateList=str(consultationStateList)
    ...
    def testRecieveOrNot(self):
        ...
        # 设置参数
        consultationid=ReadConfig.get_case('consultationid')
        customerid=ReadConfig.get_customer('customerid')
        ParamJson={"consultationId":consultationid,
                   "consultationStateList":self.consultationStateList,
