...
xls = common.get_xls("pc.xlsx", "修改删除常用回复")  # excel取值
...
class ChangeReply(unittest.TestCase):
    def setParameters(self, case_name, url, method,id,isValid,replyContent, msg ):
        self.case_name = str(case_name)
        self.method = str(method)
        self.isValid = str(isValid)

        self.replyContent = str(replyContent)
        self.msg = str(msg)
    ...
    def testChangereply(self):
        ...
        replyid=ReadConfig.get_im('replyid')
        paramJson ={"id":replyid,"replyContent":self.replyContent,"isValid":self.isValid}
        data = {
