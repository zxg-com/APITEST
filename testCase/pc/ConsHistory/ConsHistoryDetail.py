...
xls = common.get_xls("pc.xlsx", "咨询历史详情")  # excel取值
...
class ConsHistoryDetail(unittest.TestCase):
    def setParameters(self, case_name, url, method, fromAccount,receiveAccount,firstResult,maxResult,sortType,isValid,msg):
        self.case_name = str(case_name)
        self.url = str(url)
        self.method = str(method)
        self.return_json = None  # 返回值
        self.info = None  # json结果
        self.fromAccount = str(fromAccount)
    ...
    def testConsHistoryDetail(self):
        # 拼接url，也可以从excel中获取
        ...
        # 设置参数
        receiveAccount = "0_"+ReadConfig.get_case('historycaseid')
        print(receiveAccount)
        paramJson = {"fromAccount":self.fromAccount, "receiveAccount":receiveAccount,
                     "firstResult":self.firstResult, "maxResult":self.maxResult,
                     "sortType":self.sortType, "isValid":self.isValid,
        }
        data = {
