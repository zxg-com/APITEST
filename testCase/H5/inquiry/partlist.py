...
xls = common.get_xls("h5.xlsx", "选择部位")   #excel取值
...
class partlist(unittest.TestCase):
    def setParameters(self, case_name,url, method, parentId	,isValid	,firstResult	,maxResult	,directionId	,positionIdList,msg):
        ...
        self.directionId = str(directionId)
        self.positionIdList = str(positionIdList)
    ...
    def testpartlist(self):
        ...
        # 设置参数
        paramJson = {"parentId":self.parentId,"isValid":self.isValid,"firstResult":self.firstResult,"maxResult":self.maxResult,
                     "directionId":self.directionId,"positionIdList":self.positionIdList}
        data = {
