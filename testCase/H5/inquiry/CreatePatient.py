...
xls = common.get_xls("h5.xlsx", "新建患者")   #excel取值

ReadConfig = readConfig.ReadConfig()
configHttp = ConfigHttp.ConfigHttp()
info = {}
customer_id = ReadConfig.get_patient('patient_customerid')

@paramunittest.parametrized(*xls)
class CreatePatient(unittest.TestCase):
    def setParameters(self, case_name,url, method,customerId	,patientName	,patientSex	,patientRelationId	,mobile	,provinceId	,province	,cityId	,city	,districtId	,district	,patientBirthday	,certificateId	,certificateCode, msg):
        ...
        self.patientSex=str(patientSex)
        self.patientRelationId = str(patientRelationId)
        ...
        self.patientBirthday = str(patientBirthday)
        self.certificateId = str(certificateId)
        self.certificateCode=str(certificateCode)
    ...
    def testCreatePatient(self):
        ...
        # 设置参数
        paramJson ={"customerId":customer_id,
                    "patientName":self.patientName,
                    "patientSex":self.patientSex,
                    "patientRelationId":self.patientRelationId,
                    "mobile":self.mobile,
                    ...
                    "certificateCode":self.certificateCode}
        data = {
    ...
    def checkResult(self):
        ...
        ReadConfig.set_patient('patientid',self.info['responseObject']['responsePk'])  #患者ID存入配置文件
