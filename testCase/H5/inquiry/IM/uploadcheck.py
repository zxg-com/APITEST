...
xls = common.get_xls("h5.xlsx", "上传检查检验")   #excel取值
...
class uploadcheck(unittest.TestCase):
    def setParameters(self, case_name,url, method,caseId,idList, msg):
    ...
    def testuploadcheck(self):
        ...
        s=businessCommon.uploadPic(picName)
        if self.idList=="":
            self.idList = s[0]
        paramJson = {"caseId":self.caseId,
                     "idList":self.idList
                     }
        data = {
