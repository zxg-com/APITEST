...
xls = common.get_xls("h5.xlsx", "今日剩余名额")   #excel取值
...
class A(unittest.TestCase):
    def setParameters(self, case_name,url, method,customerId,caseId,patientId,orderType,orderSourceType,sortType ,msg):
        ...
        self.orderSourceType = str(orderSourceType)
    ...
    def test(self):
        ...
        # 设置参数
        if self.customerId=="":
            self.customerId=ReadConfig.get_doctor("doctor_customerid")
        if self.caseId=="":
            self.caseId=ReadConfig.get_case("caseid")
        if self.patientId=="":
            self.patientId=ReadConfig.get_patient('patientid')
        paramJson = {"customerId":self.customerId,"caseId":self.caseId,"patientId":self.patientId,
                     "orderType":self.orderType,"orderSourceType":self.orderSourceType,"sortType":self.sortType
                     }
        data = {
